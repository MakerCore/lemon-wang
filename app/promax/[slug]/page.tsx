...
export default function ProMaxPostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] px-6 py-16 max-w-3xl mx-auto">
      <a
        href="/promax"
        className="text-[#999] font-mono text-xs hover:text-[#CC0000] transition-colors mb-12 block"
      >
        ← PROMAX&apos;S
      </a>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          {tags.map(tag => (
            <span
              key={tag}
              className="inline-block px-2 py-0.5 rounded text-[10px] font-bold tracking-wide font-mono text-[#CC0000] border border-[#CC0000]"
            >
              {tag.toUpperCase()}
            </span>
          ))}
      ...
      <article className="
        ...
        prose-em:text-[#999]
        prose-blockquote:border-l-[#CC0000] prose-blockquote:text-[#999] prose-blockquote:not-italic prose-blockquote:text-xs prose-blockquote:my-6
        prose-hr:border-white/10 prose-hr:my-8
        prose-img:rounded-lg prose-img:my-8
        prose-code:text-[#CC0000] prose-code:text-[13px] prose-code:bg-white/5 prose-code:px-1 prose-code:rounded
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {otherPosts.map(post => (
              <Link
                ...
                className="group block border border-white/10 rounded-lg p-5 hover:border-[#CC0000]/40 transition-colors"
              >
                {post.tags.length > 0 && (
                  <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold tracking-wide font-mono text-[#CC0000] border border-[#CC0000] mb-3 block">
                    {post.tags[0].toUpperCase()}
                  </span>
                )}
                <h3 className="font-mono text-white text-sm group-hover:text-[#CC0000] transition-colors mb-2 leading-snug">
                  {post.title}
                </h3>
